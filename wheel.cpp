...
void Wheel::stageUpdate()
{
    if (_ticketUsed < 10)
        _stage = 1;
    else if (_ticketUsed < 30)
        _stage = 2;
    else if (_ticketUsed < 60)
        _stage = 3;
    else if (_ticketUsed < 100)
        _stage = 4;
    else if (_ticketUsed < 150)
        _stage = 5;
    else if (_ticketUsed < 210)
        _stage = 6;
    else if (_ticketUsed < 290)
        _stage = 7;
    else if (_ticketUsed < 390)
        _stage = 8;
    else
        _stage = 9;
}

void Wheel::spinJackpot()
{
    const auto& currentJackpot = _jackpot.at(_stage);

    const int x = distribution(gen);

    int min = 1;

    for (const auto& [name, reward] : currentJackpot)
    {
        const int max = reward.probability;

        if (min <= x && x < max)
        {
            _totalGain[name] += reward.quantity;
            //std::cout << "jackpot: " << name << " | Quantity: " << reward.quantity << '\n';
            return;
        }
...
void Wheel::spin(int i)
{
    for (int j = 0; j < i; ++j)
    {
        ...
        for (const auto& [name, reward] : _roll)
        {
            const int max = reward.probability;

            if (min <= x && x < max)
            {
                //std::cout << "spinned: " << name << " | Quantity: " << reward.quantity << '\n';
...
void Wheel::printLoot() const
{
    std::cout << "\n===== TOTAL LOOT =====\n";

    for (const auto& [name, quantity] : _totalGain)
    {
        std::cout
            << name
            << " x"
